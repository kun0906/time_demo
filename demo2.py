...
def foo():
    ...
    print(np.mean(cprofile_process_times), np.std(cprofile_process_times)) #, cprofile_process_times)
...
with Parallel(n_jobs=5, verbose=0, backend='loky') as parallel:
    res = parallel(delayed(foo)() for _ in combs)
